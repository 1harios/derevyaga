 ...
 *
 * Без них адаптер выключен: заявки остаются в логе сервера, сайт работает.
 */
...
/**
 * Отправка сделки. Ошибки не бросает — только сообщает результат:
 * заявка к этому моменту уже зафиксирована в логе сервера, и падение
 * CRM не должно ломать ответ клиенту. Очередь с ретраями — третья итерация.
 */
